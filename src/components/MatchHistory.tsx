  ...
  return (
    <div className="group relative">
      ...
      <div className="pointer-events-none absolute left-1/2 z-50 hidden w-64 -translate-x-1/2 rounded-2xl border border-zinc-800 bg-zinc-950/95 p-3 text-xs shadow-xl backdrop-blur group-hover:block -top-2 -translate-y-full">
        ...
        {!body && gold == null ? <div className="mt-1 text-[11px] text-zinc-500">Item #{id}</div> : null}
      </div>
    </div>
  );
}
...
function prettyPos(teamPosition?: string | null) {
  const p = String(teamPosition ?? "").toUpperCase().trim();
  if (!p || p === "NONE" || p === "INVALID") return null;
...
function RuneIcon({
  ...
  const src = `https://ddragon.leagueoflegends.com/cdn/img/${rune.icon}`;
  return (
    <img
      src={src}
      alt={rune.name || title}
...
export default function MatchHistory({
  ...
  // ✅ IMPORTANT: when Server Components refresh and pass new initialMatches/initialCursor,
  // sync your local state so you don't need F5.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`https://ddragon.leagueoflegends.com/cdn/${ddragonVersion}/data/en_US/runesReforged.json`, {
      ...
      .then((arr) => {
        ...
        const runeOut: Record<string, RuneInfo> = {};
        const styleOut: Record<string, RuneInfo> = {};

        for (const style of arr) {
          if (style?.id != null && style?.icon) {
            styleOut[String(style.id)] = {
      ...
      {empty ? (
        ...
        <div className="space-y-3">
          {items.map((m) => {
            ...
            const isArena = m.queueId != null && ARENA_QUEUES.has(m.queueId);

            const side = !isArena ? sideLabel(m.teamId ?? null) : null;
            const pos = !isArena ? prettyPos(m.teamPosition ?? null) : null;
            ...
            const primaryRune = m.primaryRune != null ? runeMap[String(m.primaryRune)] ?? null : null;
            const subStyle = m.subStyle != null ? styleMap[String(m.subStyle)] ?? null : null;
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {pos ? <Pill className="border-zinc-800 bg-zinc-950/30 text-zinc-200">{pos}</Pill> : null}
                        {side ? <Pill className="border-zinc-800 bg-zinc-950/30 text-zinc-300">{side}</Pill> : null}

                        <Pill className="border-zinc-800 bg-zinc-950/30 text-zinc-300">{dur}</Pill>
                      </div>
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        ...
                        {champName ? <div className="text-xs text-zinc-400 truncate">• {champName}</div> : null}
                      </div>
                  ...
                  <div className="shrink-0 text-right text-xs text-zinc-500 tabular-nums space-y-1">
                    <div>{m.cs ? <span className="text-zinc-300"> CS:{m.cs}</span> : "—"}</div>
                    <div>
                      Gold: <span className="text-zinc-300">{m.gold != null ? m.gold.toLocaleString() : "—"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="hidden sm:block h-7 w-px bg-zinc-800" />

                    <div className="flex items-center gap-2">
                      <RuneIcon rune={primaryRune} ddragonVersion={ddragonVersion} title="Primary Rune" />
                      <RuneIcon rune={subStyle} ddragonVersion={ddragonVersion} title="Secondary Style" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 sm:justify-end">
                    {m.items?.length ? (
                      m.items.slice(0, 7).map((id, idx) => {
                        ...
                        return <ItemIcon key={`${m._id}-${id}-${idx}`} id={id} url={url} info={info} />;
                      })
                    ) : (
