// src/app/api/p/[gameName]/[tagLine]/match/[matchId]/route.ts
...
export async function GET(
    ...
    try {
        ...
        const blue = participants.filter((p) => p.teamId === 100).sort(sortByRole);
        const red = participants.filter((p) => p.teamId === 200).sort(sortByRole);
